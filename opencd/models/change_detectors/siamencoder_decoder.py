...
@MODELS.register_module()
class SelfDistillSiamEncoderDecoder_ChangeStar(SiamEncoderDecoder):
    def __init__(self,
                 distill_loss,               # 蒸馏损失函数配置
                 backbone: ConfigType,
                 decode_head: ConfigType,
    ...
    def loss(self, inputs: Tensor, data_samples: SampleList) -> dict:
        ...
        for i, data_sample in enumerate(data_samples):
            ...
            with torch.no_grad():  # 确保教师模型不更新梯度
                if change_area_ratio < 0.974:
                    teacher_input = inputs[i:i+1]
                    teacher_s_inputs.append(teacher_input)
                    teacher_s_data_samples.append(data_sample)
                    teacher_output = self.teacher_s.decode_head.forward(self.teacher_s.extract_feat(teacher_input))
                else:
                    teacher_input = inputs[i:i+1]
                    teacher_l_inputs.append(teacher_input)
                    teacher_l_data_samples.append(data_sample)
                    teacher_output = self.teacher_l.decode_head.forward(self.teacher_l.extract_feat(teacher_input))
        ...
        # 对教师模型计算交叉熵损失
        if teacher_l_inputs:
            teacher_l_inputs = torch.cat(teacher_l_inputs, dim=0)
            teacher_l_feats = self.teacher_l.extract_feat(teacher_l_inputs)
            loss_teacher_l = self._decode_head_forward_train(teacher_l_feats, teacher_l_data_samples)
            losses.update({f"teacher_l_{k}": v for k, v in loss_teacher_l.items()})

        if teacher_s_inputs:
            teacher_s_inputs = torch.cat(teacher_s_inputs, dim=0)
            teacher_s_feats = self.teacher_s.extract_feat(teacher_s_inputs)
            loss_teacher_s = self._decode_head_forward_train(teacher_s_feats, teacher_s_data_samples)
...
@MODELS.register_module()
class SelfDistillSiamEncoderDecoder(SiamEncoderDecoder):
    def __init__(self,
                 distill_loss,               # 蒸馏损失函数配置
                 backbone: ConfigType,
                 decode_head: ConfigType,
